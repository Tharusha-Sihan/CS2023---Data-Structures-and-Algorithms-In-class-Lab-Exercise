...
struct Graph{
    ...
    void print(){
        ...
        for (int i=0; i<n; i++){
            cout << "neighbours of node "<< nodes[i].label<< " :";
            for( int neighbour :nodes[i].neighbours){   // printing neighbours of each node
                cout<< neighbour<< " ";
            }
            cout << endl;    // printing each node in seperate line
        }
    }
};
